...
train_data_path = r'C:\Users\abc\Downloads\课程设计（数字工程）\自处理后的数据集和代码\DFC22-split\train'
val_data_path = r'C:\Users\abc\Downloads\课程设计（数字工程）\自处理后的数据集和代码\DFC22-split\val'
test_data_path = r'E:\zj-dataset\dataset\DFC22-split\data\test'
# 参数的保存位置
time = str(datetime.datetime.now().strftime('%m%d%H%M')) #获取当前月日时分
state_dict_path =os.path.join("save_pth2", time)
...
# ===============================数据集的加载=============================
train_dataloader = DataLoader(MyDataset(train_data_path,mode='train'), batch_size=4, shuffle=True,drop_last=True)
val_dataloader = DataLoader(MyDataset(val_data_path,mode='val'), batch_size=4, shuffle=False,drop_last=True)
# ==============================模型及其相关工具的实例化=============================
